...
const articles: Record<string, { title: string; date: string; tags: string[]; content: string }> = {
          ...
          if (block.startsWith("- ")) {
            return (
              <ul key={i} className="space-y-2 pl-4">
                {block.split("\n").map((item, j) => (
                  <li key={j} className="text-muted">{item.replace(/^- \*\*(.+?)\*\*[:：]/, "$1: ").replace(/^- /, "")}</li>
                ))}
              </ul>
            );
          }
          if (block.match(/^\d\. /)) {
            return (
              <ol key={i} className="space-y-2 pl-4 list-decimal list-inside">
                {block.split("\n").map((item, j) => (
                  <li key={j} className="text-muted">{item.replace(/^\d+\. /, "")}</li>
                ))}
              </ol>
            );
          }
          return <p key={i} className="text-muted leading-relaxed">{block}</p>;
        })}
      </div>
    </article>
  );
